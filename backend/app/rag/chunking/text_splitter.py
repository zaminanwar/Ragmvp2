...
def get_chunker(strategy: str = "recursive", **kwargs) -> BaseChunker:
    ...
    from app.rag.chunking.hierarchical import HierarchicalChunker
    ...
    # HierarchicalChunker uses different param names
    if strategy == "hierarchical":
        return chunker_cls(
            parent_size=kwargs.get("chunk_size", 1024),
            child_size=kwargs.get("chunk_size", 512) // 2,
            parent_overlap=kwargs.get("chunk_overlap", 100),
            child_overlap=kwargs.get("chunk_overlap", 50) // 2,
        )
