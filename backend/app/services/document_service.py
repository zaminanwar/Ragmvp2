...
class DocumentService:
    ...
    async def upload_and_process(
        ...
        try:
            ...
            # 4b. Contextual embeddings (if enabled)
            embedding_provider = get_embedding_provider()
            enable_contextual = getattr(
                await self._get_workspace(workspace_id), "enable_contextual_embeddings", False
            )
            if enable_contextual:
                from app.rag.chunking.contextual import add_contextual_headers
                from app.rag.llm.factory import get_llm_provider
                ctx_llm = get_llm_provider()
                chunks = await add_contextual_headers(chunks, text, ctx_llm)
                logger.info("contextual_headers_applied", doc_id=str(doc.id))

            # 5. Embed (use contextual content if available)
            texts = [
                c.metadata.get("embedding_content", c.content) for c in chunks
            ]
    ...
    async def _get_workspace(self, workspace_id: uuid.UUID):
        """Fetch workspace for feature flag checks."""
